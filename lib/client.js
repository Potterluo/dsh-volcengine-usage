...
`
    ...
    function apply(ctx) {
      ...
      function render() {
        ...
        html += '<div class="vu-foot"><span>更新 '+ts+'</span><span id="vu-cd" style="font-variant-numeric:tabular-nums">⟳ '+Math.max(countdown,0)+'s</span></div>'
      ...
      function bindUI() {
        ...
        // Refresh button — countdown span is clickable
        var cd=panel.querySelector('#vu-cd'); if(cd) cd.addEventListener('click',function(){ refresh() })
        // Limit config
      ...
      // 自动刷新：每 60s 一次 + 切回页面（tab 恢复可见）立即刷新
      var autoInt = setInterval(function(){ if(open&&!refreshing) refresh() }, 60000)
      // 倒计时显示（footer 的 ⟳ 按钮文案）
      var cdInt = setInterval(function(){
        if(!open) return
        countdown--
        var cd = panel.querySelector('#vu-cd')
        if (cd) cd.textContent = '⟳ ' + Math.max(countdown,0) + 's'
        if (countdown <= 0) countdown = 60
      }, 1000)
      document.addEventListener('visibilitychange', function(){
        if (document.visibilityState === 'visible' && open && !refreshing) refresh()
      })

      return function dispose(){ clearInterval(autoInt); clearInterval(cdInt); host.remove(); style.remove() }
    }
